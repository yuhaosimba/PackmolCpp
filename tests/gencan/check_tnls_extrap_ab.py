...
def parse_output(text: str) -> dict[str, float | int]:
    ...
    for line in text.splitlines():
        ...
        if tokens and tokens[0] == "mode" and len(tokens) >= 20:
            values["mode"] = int(tokens[1])
            values["inform"] = int(tokens[3])
            values["fcnt"] = int(tokens[5])
            values["gcnt"] = int(tokens[7])
            values["intcnt"] = int(tokens[9])
            values["exgcnt"] = int(tokens[11])
            values["exbcnt"] = int(tokens[13])
            values["f"] = float(tokens[15])
            values["xsum"] = float(tokens[17])
            values["gnorm2"] = float(tokens[19])
    return values
...
def run_probe(probe: pathlib.Path, mode: str) -> dict[str, float | int]:
    ...
    if completed.returncode != 0:
        print(f"tnls extrap probe failed for mode={mode}", file=sys.stderr)
    ...
    required = ("inform", "fcnt", "gcnt", "intcnt", "exgcnt", "exbcnt", "f", "xsum", "gnorm2")
...
def main() -> int:
    ...
    for key in ("inform", "fcnt", "gcnt", "intcnt", "exgcnt", "exbcnt"):
